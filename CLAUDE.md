...
`pushClaimHeld` gates **`updatePushIframe()` and nothing else** — the claim decides which tab owns the webcam, not whether the stream exists. In particular `sendPresence()` must keep advertising the `streamId`: a non-claiming tab sending `''` would fight the claiming tab over the GM's single card (both heartbeat under the same `charId`), adding and removing its camera iframe every 5s. For the same reason `selfStreamLive()` is not gated on it — a non-pushing tab views its sibling's stream like any other viewer. The Caméras tab says which tab is broadcasting, as information rather than a warning.
