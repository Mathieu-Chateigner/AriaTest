...
**Only one tab pushes per character.** The push stream ID is a pure function of `charId`, so two tabs on the same character would publish into the same room under the same ID. One tab holds a timestamped claim in `aria-push-claim-{charId}` and renews it on the existing 5s presence tick (`refreshPushClaim()`); `releasePushClaim()` runs on `switchCharacter()` and `beforeunload` so the survivor takes over on its next tick instead of waiting out `PUSH_CLAIM_TTL` (12s, which is what covers a crashed tab).

The GM has the mirror of this: `gmDerivedStreamId()` is a pure function of `campaignId`, so two GM tabs on one campaign collide the same way. `aria-gm-push-claim-{campaignId}` is keyed by a per-tab `gmTabId` (sessionStorage, so a reload reclaims its own claim), renewed on the existing **8s** `gm-presence` tick — the only timer that runs exactly when the GM has a room — and released by `switchCampaign()` and `beforeunload`. `GM_PUSH_CLAIM_TTL` is 30s, matching `PRESENCE_TIMEOUT`: a tab that stopped renewing has also stopped broadcasting, so there is no reason to declare it dead sooner. `updateGMPushIframe()` splits the two questions it now has to answer — `streamLive` (is the stream published at all, by either tab?) drives the "Votre caméra" preview, `shouldPush` (`streamLive && gmPushClaimHeld`) drives the push frame.

`pushClaimHeld` / `gmPushClaimHeld` gate **the push iframe and nothing else** — the claim decides which tab owns the webcam, not whether the stream exists. In particular `sendPresence()` must keep advertising the `streamId`: a non-claiming tab sending `''` would fight the claiming tab over the GM's single card (both heartbeat under the same `charId`), adding and removing its camera iframe every 5s. `gmAdvertisedStreamId()` follows the same rule on the GM side — gated on `gmCameraOff`, never on the claim, or the MJ tile would flip every 8s for every player. For the same reason `selfStreamLive()` and the GM's `streamLive` are not gated on it: a non-pushing tab views its sibling's stream like any other viewer. The Caméras tab says which tab is broadcasting, as information rather than a warning.

The accepted cost on both sides: if the claim-holding tab **crashes** (no `beforeunload`), the stream ID stays advertised while nobody pushes it — up to ~17s (player) or ~38s (GM) before the survivor takes over — so the tile is black in that window. The alternative, gating the advertised ID on the claim, trades that rare case for a permanent flicker whenever two tabs are open.
...
```
`gmSpotlightCharId` is cleared (and `spotlight: null` published) when the spotlighted player is dropped by the **`leave`** handler. It used to be cleared only by clicking ☀ again or switching campaign, so a spotlight on a departed player stayed armed forever — invisible, since the card carrying the ☀ state is gone — and silently re-applied if that player came back. Deliberately **not** done by `sweepOfflinePlayers()`: `leave` is an explicit departure, while a silent sweep is usually a network blip the spotlight should survive. The cost is that a player refreshing their page (`beforeunload` publishes `leave` too) loses the spotlight and the GM re-clicks.
