...
// ═══════════════════════════════════════════
let splitRatio = parseInt(localStorage.getItem('aria-split-ratio') || '50', 10) || 50;
...
function _dockMove(e) {
    ...
    if (overlay) {
        if (side) {
            ...
            const mid = (region2.left + region2.right) / 2;
            overlay.style.display = 'flex';
            overlay.style.left = (side === 'left' ? region2.left : mid) + 'px';
            overlay.style.top = region2.top + 'px';
            overlay.style.width = ((region2.right - region2.left) / 2) + 'px';
            overlay.style.height = (region2.bottom - region2.top) + 'px';
...
function _dividerEnd() {
    ...
    document.body.classList.remove('dock-dragging');
    localStorage.setItem('aria-split-ratio', String(splitRatio));
}
function resetSplitRatio() {
    splitRatio = 50;
    localStorage.setItem('aria-split-ratio', '50');
    updateSplitChrome();
}
